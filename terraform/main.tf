...
# Project: Jit TV (Channel Branding · Regional Broadcaster)
# ------------------------------------------------------------------------------
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "Jit TV"
      Network     = "Channel Branding · Regional Broadcaster"
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Jit TV media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "jit_tv_channel_branding_pipeline-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-jit-tv-channel-branding-pipeline-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "jit_tv_channel_branding_pipeline-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "jit_tv_channel_branding_pipeline-post-production-policy"
  description = "Scoped S3 and KMS permissions for Jit TV post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/jit_tv_channel_branding_pipeline"
